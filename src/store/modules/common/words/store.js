...
// getters
const getters = {
  getAllWords (state, getters, rootState, rootGetters) { // 获取所有单词
    return [...state.words]
  },
  getWordsIndexItem (state, getters, rootState, rootGetters) { // 通过索引获取某一项单词信息
    return (index) => {
      return [...state.words][index]
    }
  },
  getWordsIdItem (state, getters, rootState, rootGetters) { // 通过id获取某一项单词信息
    return (id) => {
  ...
  getWordItem (state, getters, rootState, rootGetters) { // 通过单词获取某一项单词信息
    return (word) => {
      return [...state.words].find(w => {
        return w.word === word
      })
  ...
  getRandomWordItem (state, getters, rootState, rootGetters) { // 随机获取某一项单词信息
    return (idx) => {
...
const actions = {
  getWords (context) { // 请求works
    context.commit(types.DATA_RESET) // 数据重置
    return new Promise((resolve, reject) => {
      REQUEST.get('words', null, r => {
