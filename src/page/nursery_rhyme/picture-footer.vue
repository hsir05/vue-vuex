<template lang="html">
  <footer class="picbook-footer">
    ...
    <div  class="btn-cont-img white-bg audio" @click="panelAudio">
      <img src="static/img/btn_sound.png" alt="">
    </div>
  </footer>
</template>
...
export default {
  computed: {
    ...mapState('nurseryRhyme', ['playState']),
    ...mapGetters('nurseryRhyme', ['isCoverPic'])
  },
  methods: {
    ...mapMutations('nurseryRhyme', {}),
    ...mapActions('nurseryRhyme', ['lastPicBooks', 'nextPicBooks']),
    panelAudio () {
      console.log(this.playState)
      switch (this.playState) {
        case 'init':
          this.$store.commit('nurseryRhyme/PLAY_STATE', { status: 'play' })
          break
        case 'pause':
          this.$store.commit('nurseryRhyme/PLAY_STATE', { status: 'play' })
          break
        case 'play':
          this.$store.commit('nurseryRhyme/PLAY_STATE', { status: 'pause' })
          break
        case 'ended':
          this.$store.commit('nurseryRhyme/PLAY_STATE', { status: 'init' })
          break
        default:
      }
    }
